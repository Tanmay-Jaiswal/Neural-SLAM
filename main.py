...
def calc_rewards(current_explored_area, previously_explored_area):
    ...
    for a_ix, a in enumerate(current_explored_area):
        ...
        new_explored_area[a_ix] = ((a - z - previously_explored_area) > 0).float()
    rewards = new_explored_area.sum(dim=tuple(range(1,new_explored_area.dim()))) * 0.005 # actually 0.0005 but I modified it
    return rewards
                            ...
                            if l_step == args.num_local_steps - 1:
                                ...
                                for e in range(num_scenes):
                                    full_map[e, :4, lmb[e, 0]:lmb[e, 1], lmb[e, 2]:lmb[e, 3]] = \
                                        local_map[e]
                                    full_pose_cpu[e] = local_pose_cpu[e] + origins[e]
